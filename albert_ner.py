...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        return ["O", "B-460", "I-460", "B-461", "I-461", "B-462", "I-462", "B-463", "I-463", "B-464", "I-464", "X", "[CLS]", "[SEP]"]
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        if ex_index % 10000 == 0:
            logger.info(f"Writing example {str(ex_index)} of {str(len(examples))}")
...
def main(_):
    # tf.compat.v1.logging.set_verbosity(logger.INFO)
